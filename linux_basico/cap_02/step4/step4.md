...
Usamos dois argumentos para observar melhor as coisas. Podemos especificar exatamente o tempo de modificação adicionando `u`{{}} à lista de argumentos. Mas, por favor, lembre-se de que, para imprimir essa informação corretamente, você deve usar `t`{{}} com outra opção (neste caso, `u`{{}}).
...
Antes de usarmos o próximo comando, há uma opção a mais que precisamos aprender. Esse argumento é `--human-readable`{{}}, ou melhor, `-h`{{}}.
...
Ok, vamos tentar ordenar com a opção `h`{{}}:
